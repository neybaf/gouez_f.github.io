...
ui <- fluidPage(
  ...
  sidebarLayout(
    sidebarPanel(
      ...
      selectInput("group_compare", "Choisissez un groupe à comparer:", choices = unique(listedeputes$libelle_groupe)),
      selectInput("group_approx", "Choisissez un groupe pour le rapprochement:", choices = unique(listedeputes$libelle_groupe)),
...
server <- function(input, output) {
  votesData <- eventReactive(input$search, {
    files <- list.files(path = "//Users/fabiengouez/Desktop/R_ET_ET_RMD/pol_depute/src16/AMO20xml/votes/", pattern = "*.xml", full.names = TRUE) # Adapter le chemin vers vos fichiers XML
    votes_list <- lapply(files, function(file) {
      tryCatch({
        ...
        message(e)
        return(data.frame(dateScrutin = character(), libelle = character(), acteurRef = character(), vote = character(), organeRef = character(), stringsAsFactors = FALSE))
      })
    })
    ...
    votes <- votes %>%
      ...
      group_by(organeRef, acteurRef) %>%
      mutate(
        vote_diff_group = ifelse(vote != organeRef, 1, 0),
